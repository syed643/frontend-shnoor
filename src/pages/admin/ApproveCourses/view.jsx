...
const ApproveCoursesView = ({
  ...
  return (
    <div className="h-[calc(100vh-6rem)] flex flex-col p-2 bg-slate-50 font-sans w-full">
      ...
      {}
      <div
        className={`flex flex-1 gap-6 overflow-hidden w-full ${selectedCourse ? "grid grid-cols-1 lg:grid-cols-[1fr_500px]" : ""}`}
      >
        {}
                ...
                {pendingCourses.length === 0 ? (
                  ...
                  pendingCourses.map((course) => (
                    <tr
                      ...
                      className={`cursor-pointer transition-all hover:bg-slate-50 group border-l-4 ${
                    ...
                    >
                      <td className="py-4 px-6">
                        <div
                          className={`font-bold text-base transition-colors ${selectedCourse?.courses_id === course.courses_id ? "text-[var(--color-indigo-600)]" : "text-slate-800"}`}
                        >
                          {course.title}
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-6 text-right">
                        <button
                          className={`text-xs font-bold border px-3 py-1.5 rounded-lg shadow-sm transition-all ${
                            selectedCourse?.courses_id === course.courses_id
        ...
        {selectedCourse && (
          <div className="bg-white rounded-2xl shadow-xl border border-slate-200 flex flex-col overflow-hidden animate-slide-in-right h-full">
            ...
            <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-start gap-4">
              <div>
                <h3 className="text-xl font-bold text-slate-900 leading-tight mb-2 tracking-tight">
                  {selectedCourse.title}
                </h3>
                <p className="text-xs text-slate-500 font-bold uppercase tracking-wide">
                  <span className="text-[var(--color-indigo-600)]">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
              <div>
                <h4 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-2">
                  Description
                </h4>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {modules.map((m, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-xl shadow-sm hover:shadow-md transition-shadow group"
                    >
                      <div
                        className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${
                          m.type === "video"
                            ? "bg-rose-50 text-rose-500"
                            : "bg-indigo-50 text-[var(--color-indigo-600)]"
                        }`}
                      >
                        {m.type === "video" ? (
                      ...
                      {m.type === "text_stream" || m.type === "video" ? (
                        <button
                          onClick={() => {
                            setPreviewModuleId(m.module_id);
                            setPreviewUrl(m.content_url);
                            setPreviewModuleType(m.type);
                          }}
                          className="text-xs font-bold text-[var(--color-indigo-600)] hover:underline px-3 bg-indigo-50 py-1.5 rounded-md flex items-center gap-1"
                        >
                          <Eye size={12} /> View
                        </button>
                      ) : (
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 flex gap-3">
              <button
                ...
                onClick={() =>
                  handleAction(selectedCourse.courses_id, "rejected")
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  handleAction(selectedCourse.courses_id, "approved")
                }
              >
      ...
      {previewModuleId && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl h-[85vh] flex flex-col overflow-hidden relative border border-slate-700">
