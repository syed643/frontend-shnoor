...
const avatarModules = import.meta.glob('/src/assets/bot-avatar.*', { eager: true });
...
const StudentBot = () => {
  ...
  const [messages, setMessages] = useState([
    { from: "bot", text: "Hi! I'm your Study Assistant. Ask me anything about your courses — I can help with schedules, assignments, and quick tips." },
  ]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      appendMessage({ from: "bot", text: `Failed to contact bot: ${err.message}` });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed right-6 bottom-6 z-50">
      ...
      {!open && (
        <div className="relative" onMouseEnter={() => setShowPreview(true)} onMouseLeave={() => setShowPreview(false)}>
          ...
          >
            ...
            <div className="rounded-full bg-white p-[3px] flex items-center justify-center">
              ...
              <div className="w-12 h-12 rounded-full bg-slate-900 flex items-center justify-center overflow-hidden relative">
                <img src={botAvatarUrl} alt="Study Assistant" onError={handleImgError} className="w-8 h-8 rounded-full object-cover" />
                {/* subtle outer pulse */}
                <span className="absolute inset-0 rounded-full avatar-pulse" style={{ opacity: 0.6 }} />
              </div>
            </div>
          </button>
          ...
          {/* Hover preview */}
          <div className={`absolute -right-0 -top-20 w-40 bg-white rounded-lg shadow-xl p-2 z-50 transform origin-bottom-right ${showPreview ? 'preview-fade show' : 'preview-fade'}`}>
            <div className="w-full flex items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-slate-900 overflow-hidden flex items-center justify-center">
                <img src={botAvatarUrl} alt="Preview" onError={handleImgError} className="w-10 h-10 rounded-full object-cover" />
              </div>
              <div className="flex-1">
                <div className="font-semibold text-sm">Study Assistant</div>
                <div className="text-xs text-slate-400">Click to open the assistant</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Expanded chat box */}
      {open && (
        <div className={`w-96 h-96 transition-all duration-300 shadow-2xl rounded-2xl bg-white overflow-hidden border border-slate-200 flex flex-col`}>
          <div className="flex items-center gap-3 p-4 rounded-t-2xl bg-gradient-to-r from-[#1f2937]/80 via-[#111827]/70 to-[#0f172a]/80 text-white backdrop-blur-sm shadow-2xl">
            <div className="relative">
              <div className="p-[3px] rounded-full bg-gradient-to-br from-purple-600 via-fuchsia-500 to-indigo-600">
                <div className="w-12 h-12 rounded-full bg-slate-900 flex items-center justify-center overflow-hidden">
                  <img src={botAvatarUrl} alt="Assistant" onError={handleImgError} className="w-8 h-8 rounded-full border-2 border-white shadow-md object-cover" />
                </div>
              </div>
            ...
            <div className="flex-1">
              <div className="font-semibold text-sm">Study Assistant</div>
              <div className="text-[12px] text-white/80">Here to help — try "show my assignments"</div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => setOpen(false)} className="p-2 rounded-full hover:bg-white/10" title="Close">
                <X size={16} />
              </button>
            </div>
          </div>

          <div ref={listRef} className="flex-1 p-4 overflow-auto space-y-4 bg-gradient-to-b from-white to-slate-50">
            {messages.map((m, i) => (
              <div key={i} className={`relative flex ${m.from === "user" ? "justify-end" : "justify-start"}`}>
                <div style={m.from === "user" ? { animationDelay: `${i * ANIM_BASE_DELAY}ms`, background: `linear-gradient(90deg, ${THEME.USER_MSG_START}, ${THEME.USER_MSG_END})` } : { animationDelay: `${i * ANIM_BASE_DELAY}ms`, background: THEME.BOT_BG, borderColor: THEME.BOT_BORDER }} className={`message-enter max-w-[80%] relative px-4 py-3 rounded-[18px] ${m.from === "user" ? "text-white shadow-md" : "text-blue-900 border shadow-sm"}`}>
                  {m.text}
                  <div className={`text-[11px] mt-1 ${m.from === "user" ? "text-white/70" : "text-slate-400"}`}>{/* reserved for timestamp or meta */}</div>

                  {/* bubble tail */}
                  {m.from === "bot" ? (
                    <div style={{ background: THEME.BOT_BG }} className="absolute -left-2 top-4 w-3 h-3 rotate-45 shadow-sm" />
                  ) : (
                    <div style={{ background: THEME.USER_MSG_END }} className="absolute -right-2 top-4 w-3 h-3 rotate-45" />
                  )}
                </div>
              </div>
            ))}
            ...
            {loading && (
              <div className="flex items-start gap-2">
                <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center">
                  <svg width="18" height="4" viewBox="0 0 44 10" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="5" cy="5" r="4" fill="#cbd5e1"><animate attributeName="opacity" values="0.2;1;0.2" dur="1s" repeatCount="indefinite" begin="0s" /></circle><circle cx="22" cy="5" r="4" fill="#cbd5e1"><animate attributeName="opacity" values="0.2;1;0.2" dur="1s" repeatCount="indefinite" begin="0.15s" /></circle><circle cx="39" cy="5" r="4" fill="#cbd5e1"><animate attributeName="opacity" values="0.2;1;0.2" dur="1s" repeatCount="indefinite" begin="0.3s" /></circle></svg>
                </div>
                <div className="bg-white text-slate-700 border border-slate-100 px-3 py-2 rounded-2xl shadow-sm">Typing…</div>
              </div>
            )}
          </div>

          <div className="px-4 pb-4 pt-2 border-t border-slate-100 bg-white">
            <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} className="flex items-center gap-3 relative">
              ...
              <button
                type="submit"
                disabled={loading}
                style={{ background: `linear-gradient(135deg, ${THEME.SEND_START}, ${THEME.SEND_END})` }}
            ...
            <div className="text-xs text-slate-400 mt-2">Ask doubts related to your course content</div>
          </div>
        </div>
      )}
    </div>
